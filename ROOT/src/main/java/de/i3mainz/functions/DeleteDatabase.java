...
/**
 * SERVLET DeleteDatabase
 *
 * @author Martin Unold M.Sc.
 * @author i3mainz - Institute for Spatial Information and Surveying Technology
 * @version 04.05.2015
 */
@WebServlet(name = "DeleteDatabase", urlPatterns = {"/DeleteDatabase"})
public class DeleteDatabase extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			...
			if (!gis.executeSQL("delete.sql")) {
				response.sendRedirect(Utils.REDIRECT_MAP);
			} else {
				gis.close();
				throw new Exception("Failed to execute delete.sql");
			}
			gis.close();
		} catch (Exception e) {
